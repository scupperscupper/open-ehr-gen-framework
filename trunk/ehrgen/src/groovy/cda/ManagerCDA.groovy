...
class ManagerCDA {
   ...
   String createCDA( Composition comp )
   {
      ...
      def autoresPersonas = serviceDemographic.findPersonById(uidAutor) // FIXME: es UNA sola persona
      ...
      */
      autor_assignedAuthor_assignedPerson_name_prefix = ""
      autor_assignedAuthor_assignedPerson_name_given = autoresPersonas.primerNombre
      //autor_assignedAuthor_assignedPerson_name_given += autoresPersonas.segundoNombre // TODO: generar un nuevo givem
      autor_assignedAuthor_assignedPerson_name_family = autoresPersonas.primerApellido
      //autor_assignedAuthor_assignedPerson_name_family += autoresPersonas.segundoApellido // TODO: generar un nuevo family
      ...
      comp.context.participations.each { elem ->
         ...
         // FIXME: no hay un buscar por id y rol?
         def listaPersonasPacientes = serviceDemographic.findByPersonDataAndIdAndRole(null, null, null, null,
                                                                                      null, null, idPaciente, Role.PACIENTE)
         ...
         if (listaPersonasPacientes != null) // Si se encontro algún paciente
         {
            listaPersonasPacientes.each { elemPers -> // Deberia haber un unico Paciente
			   ...
			   // No se usa Person.identities, los campos se pusieron duros en la clase
               //if (elemPers.identities != null)
               //{ // Si tiene un nombre (PersonName)
                  ...
                  recordTarget_patientRole_patient_name_given = elemPers.primerNombre// Nombres del Paciente
                  //recordTarget_patientRole_patient_name_given += elemPersName.segundoNombre // TODO: generar un nuevo given
                  recordTarget_patientRole_patient_name_family = elemPers.primerApellido // Apellidos del Paciente
                  //recordTarget_patientRole_patient_name_family += elemPersName.segundoApellido // TODO: generar un nuevo family
               //}
      ...
      // component_nonxmlbody viene con los caracteres '<'' y '>' cambiados.
      ...
      return writer.toString().replace("&lt;","<").replace("&gt;",">").replace("_DOS_PUNTOS_", ":")
   }


   /**
    * Renderea la composition a un string usando un template. 
    * @param o es la composition
    * @return String
    */
